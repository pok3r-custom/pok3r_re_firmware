...
VortexTester::VortexTester() : debug(false), nop(false){

}

VortexTester::~VortexTester(){

}

bool VortexTester::open(){
    // Try firmware vid and pid
    if(HIDDevice::open(HOLTEK_VID, VORTEX_TESTER_PID, UPDATE_USAGE_PAGE, UPDATE_USAGE)){
        LOG("Opened Vortex Tester");
        builtin = false;
        return true;
    }
    // Try builtin vid and pid
    if(HIDDevice::open(HOLTEK_VID, VORTEX_TESTER_BOOT_PID, UPDATE_USAGE_PAGE, UPDATE_USAGE)){
        LOG("Opened Vortex Tester (builtin)");
        builtin = true;
        return true;
    }
    return false;
...
bool VortexTester::enterFirmware(){
    if(!builtin){
//        LOG("In Firmware");
        return true;
    }
...
bool VortexTester::enterBootloader(){
    if(builtin){
//        LOG("In Bootloader");
        return true;
    }
...
ZString VortexTester::getVersion(){
    ZBinary data;
...
bool VortexTester::clearVersion(){
    DLOG("clearVersion");
    if(!enterBootloader())
        return false;
...
bool VortexTester::setVersion(ZString version){
...
ZBinary VortexTester::dumpFlash(){
    ZBinary dump;
...
bool VortexTester::writeFirmware(const ZBinary &fwbinin){
...
bool VortexTester::update(ZString version, const ZBinary &fwbin){
    // Reset to bootloader
    if(!enterBootloader())
        return false;
...
bool VortexTester::eraseFlash(zu32 start, zu32 length){
...
bool VortexTester::readFlash(zu32 addr, ZBinary &bin){
    DLOG("readFlash " << addr);
    ZBinary data;
...
bool VortexTester::writeFlash(zu32 addr, ZBinary bin){
...
zu32 VortexTester::crcFlash(zu32 addr, zu32 len){
    if(addr < VER_ADDR){
        ELOG("bad address");
        return 0;
    }
...
bool VortexTester::sendCmd(zu8 cmd, zu8 a1, zu16 a2, ZBinary data){
    if(data.size() > 52){
        ELOG("bad data size");
        return false;
    }
...
bool VortexTester::sendRecvCmd(zu8 cmd, zu8 a1, ZBinary &data, zu16 a2){
...
void VortexTester::decode_firmware(ZBinary &bin){
    Pok3rRGB::decode_firmware(bin);
}

void VortexTester::encode_firmware(ZBinary &bin){
