...
static u32 SD_GetResponse(u8 uResponse);
...
u32 SD_Init(void)
{
  ...
  do
  {
    ...
    uResult = SD_GetResponse(SD_IN_IDLE_STATE);
    uTimeoutCount--;
...
u32 SD_ReadBlock(u8* pBuf, u32 uReadAddr, u32 uReadSize)
{
  ...
  if (!SD_GetResponse(SD_RESPONSE_NOERR))
  {
    /* Wait for data token start byte                                                                       */
    if (!SD_GetResponse(SD_START_SINGLE_BLOCK_READ))
    {
...
u32 SD_ReadMultipleBlock(u8* pBuf, u32 uReadAddr, u32 uReadSize)
{
  ...
  while (uReadCount--)
  {
    if (!SD_GetResponse(SD_START_MULTIPLE_BLOCK_READ))
    {
      #if 0
...
static u8 SD_ReadByte(void)
{
  ...
  //SD_ClearSPIFIFO();  //Skip for speed up
...
#if 0
/*********************************************************************************************************//**
  * @brief  Write data to SD card (1 Byte).
  * @param  uData: Data to write
  * @retval None
  ***********************************************************************************************************/
static void SD_WriteByte(u8 uData)
{
  SPI_SendData(SD_SPI_PORT, uData);
...
static u32 SD_GetResponse(u8 uResponse)
{
  u32 uTimeoutCount = SD_GETRSP_CNT;

  SD_ClearSPIFIFO();

  while ((SD_ReadByte() != uResponse) && uTimeoutCount)
  {
    uTimeoutCount--;
  }
...
static u32 SD_GetDataResponse(void)
{
  ...
  //if (uTimeoutCount == 0)
  //{
  //  SDDBG("SD: SD_GetDataResponse timeout [%x]\r\n", uResult);
  //}
  ...
  if (uResult == (SD_IN_IDLE_STATE | SD_ILLEGAL_COMMAND))
  {
    ...
    if (uTimeoutCount == 0)
    {
      ...
      do
      {
        ...
        uTimeoutCount--;
      } while (SD_GetResponse(SD_RESPONSE_NOERR) && uTimeoutCount);
