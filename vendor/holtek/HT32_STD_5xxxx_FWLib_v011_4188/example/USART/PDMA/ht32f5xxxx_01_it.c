...
void PDMA_CH2_5_IRQHandler(void)
{
  ...
  if (PDMA_GetFlagStatus(PDMA_CH2, PDMA_FLAG_GE | PDMA_FLAG_TE))
  {
    /* Buffer Full ,reset RxBuffer's address                                                                */
  ...
  if (PDMA_GetFlagStatus(PDMA_CH4, PDMA_FLAG_GE | PDMA_FLAG_TE))
  {
    /* Buffer Full ,reset RxBuffer's address                                                                */
    PDMACH_RxStructure.PDMACH_DstAddr = (u32)RxBuffer;
    PDMA_Config(PDMA_CH4, &PDMACH_RxStructure);
    /* Clear interrupt flags                                                                                */
    PDMA_ClearFlag(PDMA_CH4, PDMA_FLAG_GE | PDMA_FLAG_TE);
    /*Enable Rx PDMA                                                                                        */
    PDMA_EnaCmd(PDMA_CH4, ENABLE);
  }

  if (PDMA_GetFlagStatus(PDMA_CH3, PDMA_FLAG_GE | PDMA_FLAG_TE))
  {
    /* Clear interrupt flags                                                                                */
    PDMA_ClearFlag(PDMA_CH3, PDMA_FLAG_GE | PDMA_FLAG_TE);
    USART_PDMA_TxState = USART_PDMA_IDLE;
    USART_PDMACmd(COM1_PORT, USART_PDMAREQ_TX, DISABLE);
  }

  if (PDMA_GetFlagStatus(PDMA_CH5, PDMA_FLAG_GE | PDMA_FLAG_TE))
  {
    /* Clear interrupt flags                                                                                */
    PDMA_ClearFlag(PDMA_CH5, PDMA_FLAG_GE | PDMA_FLAG_TE);
    USART_PDMA_TxState = USART_PDMA_IDLE;
    USART_PDMACmd(COM1_PORT, USART_PDMAREQ_TX, DISABLE);
  }
}
...
void PDMA_CH0_1_IRQHandler(void)
{
  ...
  if (PDMA_GetFlagStatus(PDMA_CH0, PDMA_FLAG_GE | PDMA_FLAG_TE))
  {
    /* Buffer Full ,reset RxBuffer's address                                                                */
    PDMACH_RxStructure.PDMACH_DstAddr = (u32)RxBuffer;
    PDMA_Config(PDMA_CH0, &PDMACH_RxStructure);
    /* Clear interrupt flags                                                                                */
    PDMA_ClearFlag(PDMA_CH0, PDMA_FLAG_GE | PDMA_FLAG_TE);
    /*Enable Rx PDMA                                                                                        */
    PDMA_EnaCmd(PDMA_CH0, ENABLE);
  }

  if (PDMA_GetFlagStatus(PDMA_CH1, PDMA_FLAG_GE | PDMA_FLAG_TE))
  {
    /* Clear interrupt flags                                                                                */
