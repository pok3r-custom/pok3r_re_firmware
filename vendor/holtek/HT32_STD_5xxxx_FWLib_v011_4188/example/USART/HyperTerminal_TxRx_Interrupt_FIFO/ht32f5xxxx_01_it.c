...
void HTCFG_USART_IRQHandler(void)
{
  ...
  if (USART_GetFlagStatus(HTCFG_USART, USART_FLAG_RXDR))
  {
    ...
    UR0RxBuf[0] = USART_ReceiveData(HTCFG_USART);
    UR0RxBuf[1] = USART_ReceiveData(HTCFG_USART);
    UR0RxBuf[2] = USART_ReceiveData(HTCFG_USART);
    UR0RxBuf[3] = USART_ReceiveData(HTCFG_USART);
    /* Transmiting these datas                                                                              */
    USART_SendData(HTCFG_USART, UR0RxBuf[0]);
    USART_SendData(HTCFG_USART, UR0RxBuf[1]);
    USART_SendData(HTCFG_USART, UR0RxBuf[2]);
    USART_SendData(HTCFG_USART, UR0RxBuf[3]);
