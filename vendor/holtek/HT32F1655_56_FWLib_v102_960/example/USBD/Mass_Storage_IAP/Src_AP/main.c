...
int main(void)
{
  ...
  NVIC_SetVectorTable(NVIC_VECTTABLE_FLASH, 0x00002000); /* Set base location of the Vector Table at 0x2000 */

  RETARGET_Configuration();                              /* USART Related configuration                     */

  SYSTICK_ClockSourceConfig(SYSTICK_SRC_STCLK);         /* Default : 9MHz ON CHIP                           */
  SYSTICK_SetReloadValue(4500000);                      /* (4.5MHz/9MHz) = 500ms ON CHIP                    */
  ...
  while (1)                           /* Infinite loop                                                      */
  {
    ...
    {
      /*----------------------------------------------------------------------------------------------------*/
      /* Set SBVT1 as 0xFAB2 and trigger a reset to start IAP mode. SBVT registers only reset by            */
      /* Power-on-reset.                                                                                    */
      /*----------------------------------------------------------------------------------------------------*/
      HT_FLASH->SBVT[1] = 0xFAB2;
