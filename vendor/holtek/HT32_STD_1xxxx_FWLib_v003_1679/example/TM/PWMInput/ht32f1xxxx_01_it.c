...
void GPTM1_IRQHandler(void)
{
  ...
  else if (TM_GetIntStatus(HT_GPTM1, TM_INT_CH1CC) != RESET)
  {
    /* Compute wFrequency and wDutyCycle                                                                    */
    ...
    CaptureValue = TM_GetCaptureCompare1(HT_GPTM1) + SLAVE_RESTART_LATENCY;

    if (CaptureValue != 0)
    {
      /* Duty cycle computation                                                                             */
      wDutyCycle = ((TM_GetCaptureCompare0(HT_GPTM1) + SLAVE_RESTART_LATENCY) << 16) / CaptureValue;
      wDutyCycle *= 100;

      /* wFrequency computation                                                                             */
      wFrequency = SystemCoreClock / CaptureValue;
    }
    else
    {
      wDutyCycle = 0;
