/*********************************************************************************************************//**
 * @file    GPTM/MatchOutputActive/main.c
 * @version $Rev:: 929          $
...
/*********************************************************************************************************//**
  ...
  *     - Prescaler = 9
  *     - CounterReload = 65535
  *     - GPTM0_CounterClock = (GPTM0_PCLK / (Prescaler + 1)) = 7.2 MHz
...
int main(void)
{
  ...
  /* Force channel output reference to inactive level                                                       */
  GPTM_OutputModeConfig(HT_GPTM0, GPTM_CH_3, GPTM_OM_FORCED_INACTIVE);

  /* GPTM0 Match Output Active Mode configuration: Channel3                                                 */
  OutInit.Channel = GPTM_CH_3;
  OutInit.OutputMode = GPTM_OM_MATCH_ACTIVE;
  OutInit.Control = GPTM_CHCTL_ENABLE;
  OutInit.Polarity = GPTM_CHP_NONINVERTED;
  OutInit.Compare = 7200;
