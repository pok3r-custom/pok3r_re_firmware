...
static u32 iapCheckAddr(u32 saddr, u32 eaddr)
{
  if ((saddr >= IAP_FLASH_START && eaddr <= IAP_FLASH_END) ||
      (saddr >= IAP_OB_START    && eaddr <= IAP_OB_END))
  {
    return ADDRESS_OK;
  }
  else
...
u32 IAP_Go(u32 type)
{
  ...
  {
    /*------------------------------------------------------------------------------------------------------*/
    /* Check Stack Point in range (0x20000000 ~ 0x20002000)                                                 */
    /*------------------------------------------------------------------------------------------------------*/
    ...
    /*------------------------------------------------------------------------------------------------------*/
    /* Check PC in range (0x00000C00 ~ 0x7BFF)                                                              */
    /*------------------------------------------------------------------------------------------------------*/
...
static u32 IAP_Read(u32 reserved, u32 saddr, u32 eaddr)
{
  ...
  for (i = saddr; i <= eaddr; i += 4)
  {
    if (isReadOption == 1 && i >= IAP_DATA_START)
    {
        data = 0x0;
    }
    else
