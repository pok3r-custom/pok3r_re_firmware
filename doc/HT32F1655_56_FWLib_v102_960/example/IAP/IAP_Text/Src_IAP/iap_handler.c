...
void IAP_Go(void)
{
  ...
  if (SP < HT_SRAM_BASE || SP > IAP_SRAM_END)
  {
  ...
  /*--------------------------------------------------------------------------------------------------------*/
  NVIC_DisableIRQ(USART0_IRQn);   /* IAP UART SETTING: Disable UxRT interrupt to prevent unwanted interrupt */

  NVIC_SetVectorTable(NVIC_VECTTABLE_FLASH, IAP_FLASH_START); /* Set Vector Table Offset                    */
  IAP_GoCMD(IAP_FLASH_START);
...
static void IAP_Download(void)
{
  ...
  /*--------------------------------------------------------------------------------------------------------*/
  for (i = IAP_FLASH_START; i < IAP_FLASH_END; i += FLASH_PAGE_SIZE)
  {
    FLASH_ErasePage(i);
  }
  ...
  /*--------------------------------------------------------------------------------------------------------*/
  result = xmodem(IAP_Buffer, IAP_FLASH_START,  XMODEM_PROGRAM);
...
void IAP_Handler(void)
{
  ...
  /*--------------------------------------------------------------------------------------------------------*/
  /* Reset SBVT1 which set by user's application, to avoid start IAP again                                  */
  /*--------------------------------------------------------------------------------------------------------*/
  HT_FLASH->SBVT[1] = 0x00000000;
  ...
  while (1)
  {
    ...
    isFlashProtection = Option.WriteProtect[0] & ~(IAP_PAGE);
      ...
      case '2':
      {
        if (isFlashProtection != 0)
        {
          ...
          else
          {
            ...
            //Option.WriteProtect[0] &= IAP_PAGE;
