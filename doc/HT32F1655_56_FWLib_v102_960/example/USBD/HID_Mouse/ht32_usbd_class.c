...
static void USBDClass_EPT1INProcess(void)
{
  ...
  gMouseInfo.x = 0;
  gMouseInfo.y = 0;
  if (key != JOYKEY_NONE && gIsEP1INEmpty == TRUE)
  {
    switch (key)
    {
      case JOYKEY_LEFT:
    ...
    gMouseInfo.Button.bits.B1 = !GPIO_ReadInBit(HT_GPIOE, GPIO_PIN_1);
    gMouseInfo.Button.bits.B2 = !GPIO_ReadInBit(HT_GPIOE, GPIO_PIN_2);
...
static vu32 JoyKeyState(void)
{
  if (!GPIO_ReadInBit(HT_GPIOE, GPIO_PIN_4))
  {
    return JOYKEY_LEFT;
  }
  if (!GPIO_ReadInBit(HT_GPIOE, GPIO_PIN_6))
  {
    return JOYKEY_SEL;
  }
  if (!GPIO_ReadInBit(HT_GPIOD, GPIO_PIN_6))
  {
    return JOYKEY_UP;
  }
  if (!GPIO_ReadInBit(HT_GPIOE, GPIO_PIN_5))
  {
    return JOYKEY_RIGHT;
  }
  if (!GPIO_ReadInBit(HT_GPIOC, GPIO_PIN_3))
  {
    return JOYKEY_DOWN;
  }
  if (gMouseInfo.Button.bits.B1 != !GPIO_ReadInBit(HT_GPIOE, GPIO_PIN_1))
  {
    return JOYKEY_KEY1;
  }
  if (gMouseInfo.Button.bits.B2 != !GPIO_ReadInBit(HT_GPIOE, GPIO_PIN_2))
  {
    return JOYKEY_KEY2;
  }
