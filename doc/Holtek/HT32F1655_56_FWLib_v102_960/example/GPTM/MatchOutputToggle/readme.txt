/**
 @page GPTM_MatchOutputToggle

 @verbatim
 * @file    GPTM/MatchOutputToggle/readme.txt
 * @version V1.00
 * @date    2014-06-30
...
The GPTM0_PCLK frequency is set to 72 MHz and the Prescaler is 0, so the GPTM0 counter clock is 72 MHz.

The counter reload is configured to max value 0xFFFF;

The channels delta compare match value as following:
- Channel0 output 1KHz frequency, the CH0DeltaValue = (72MHz / (1KHz * 2 * (Prescaler+1))
- Channel1 output 2KHz frequency, the CH1DeltaValue = (72MHz / (2KHz * 2 * (Prescaler+1))
- Channel2 output 3KHz frequency, the CH2DeltaValue = (72MHz / (3KHz * 2 * (Prescaler+1))
- Channel3 output 4KHz frequency, the CH3DeltaValue = (72MHz / (4KHz * 2 * (Prescaler+1))
...
- GPTM/MatchOutputToggle/ht32fxxxx_it.c           Interrupt handlers
