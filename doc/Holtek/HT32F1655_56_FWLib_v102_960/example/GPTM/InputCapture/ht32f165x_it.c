/*********************************************************************************************************//**
 * @file    GPTM/InputCapture/ht32f165x_it.c
 * @version $Rev:: 929          $
...
{
  if (cap->ChannelPolarity == GPTM_CHP_NONINVERTED)
  {
    ...
    cap->ChannelPolarity = GPTM_CHP_INVERTED;
  }
  else
  {
    ...
    cap->ChannelPolarity = GPTM_CHP_NONINVERTED;
  }
}
...
void GPTM1_IRQHandler(void)
{
  ...
  if ((status & GPTM_INT_UEV) != (HT_GPTM1->INTSR & GPTM_INT_UEV))
  {
...
#if 1
  if (status & GPTM_INT_CH0CC)
  {
    u32 cap_value = GPTM_GetCaptureCompare0(HT_GPTM1);
    ...
    GPTM_ChPolarityConfig(HT_GPTM1, GPTM_CH_0, CaptureCH0.ChannelPolarity);
  }
#endif

  if (status & GPTM_INT_CH1CC)
  {
    u32 cap_value = GPTM_GetCaptureCompare1(HT_GPTM1);
    bool isCapBeforeUpdate = (update_flag && (cap_value > cnt))? TRUE : FALSE;
    CaptureProcess(&CaptureCH1, cap_value, isCapBeforeUpdate);
    GPTM_ChPolarityConfig(HT_GPTM1, GPTM_CH_1, CaptureCH1.ChannelPolarity);
  }

#if 1
  if (status & GPTM_INT_CH2CC)
  {
    u32 cap_value = GPTM_GetCaptureCompare2(HT_GPTM1);
    bool isCapBeforeUpdate = (update_flag && (cap_value > cnt))? TRUE : FALSE;
    CaptureProcess(&CaptureCH2, cap_value, isCapBeforeUpdate);
    GPTM_ChPolarityConfig(HT_GPTM1, GPTM_CH_2, CaptureCH2.ChannelPolarity);
  }
#endif

  if (status & GPTM_INT_CH3CC)
  {
    u32 cap_value = GPTM_GetCaptureCompare3(HT_GPTM1);
    bool isCapBeforeUpdate = (update_flag && (cap_value > cnt))? TRUE : FALSE;
    CaptureProcess(&CaptureCH3, cap_value, isCapBeforeUpdate);
    GPTM_ChPolarityConfig(HT_GPTM1, GPTM_CH_3, CaptureCH3.ChannelPolarity);
  }
}
